...
<script>
    ...
    function load_picture()
	{
		...
		ArrayList<String[]> picturelist = picture_list;
		for(String[] list : picturelist)
		{
			%>
			$('#contents1').append('<p>제목 : <%=list[3]%> 작성자 : <%=list[2]%> 작성일 : <%=list[1].split(" ")[0]%></p><hr>');
			$('#contents1').append('<%=list[4].replaceAll("(\r\n|\r|\n|\n\r)", "")%>'.replace(/\r/g, ''));
			<%
		}
		%>
		if(<%=picturelist.get(0)[2].equals(nick_name)%>)
		{
	...
	<div class="wrapper" >
		<div align="right">
			...
			<div id="contents1">
				...
				<input type="button" style="float:right; display:none;visibility:hidden;" value="게시글삭제" id="delete" onClick="location.href='controller.jsp?action=picture_delete&cnt=<%=picturelist.get(0)[0]%>&folder=<%=picturelist.get(0)[5]%>'">				
				<br>
				<br>
			</div>
